...
int sk_config_set_multiple_scatter_source(Config *config, int multiple_scatter_source) {
    if (config == nullptr) {
        return -1; // Error: null pointer
    }
    config->impl.set_multiple_scatter_source(static_cast<sasktran2::Config::MultipleScatterSource>(multiple_scatter_source));
    return 0; // Success
}

int sk_config_set_single_scatter_source(Config *config, int single_scatter_source) {
    if (config == nullptr) {
        return -1; // Error: null pointer
    }
    config->impl.set_single_scatter_source(static_cast<sasktran2::Config::SingleScatterSource>(single_scatter_source));
    return 0; // Success
}
