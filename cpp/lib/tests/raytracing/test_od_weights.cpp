...
namespace {
    ...
    sasktran2::Geometry1D make_geometry(
        ...
        return sasktran2::Geometry1D(
            0.4, 0.2, 6372000.0,
            grid({0.0, 800.0, 2500.0, 7000.0, 15000.0, 30000.0, 50000.0,
                  80000.0}),
            interpolation, geometry_type);
    }

    Eigen::VectorXd legacy_layer_weights(
    ...
    Eigen::MatrixXd legacy_od_matrix(
    ...
    void require_od_weights(
        ...
        for (int layer_index = 0; layer_index < ray.layers.size();
            ...
            REQUIRE(compact.isApprox(legacy.row(layer_index).transpose(),
                                     2e-13));

            const double effective_distance =
                layer.layer_distance * layer.curvature_factor;
            REQUIRE(layer.integrated_od.weights[0] +
                        layer.integrated_od.weights[1] ==
                    Catch::Approx(effective_distance).epsilon(2e-12).margin(
                        1e-9));
        ...
        for (int i = 0; i < spectra.rows(); ++i) {
            for (int j = 0; j < spectra.cols(); ++j) {
                spectra(i, j) = 1e-7 * (1.0 + i * 0.11 + j * 0.23 +
                                        i * j * 0.013);
            }
        }
...
TEST_CASE("Integrated 1D OD weights - spherical paths",
    ...
    rays.push_back(sasktran2::viewinggeometry::TangentAltitude(
                       2500.0, 0.2, 120000.0)
    ...
    rays.push_back(sasktran2::viewinggeometry::TangentAltitude(
                       7000.0, 0.2, 10000.0)
                       .construct_ray(geometry.coordinates()));
    rays.push_back(sasktran2::viewinggeometry::GroundViewingSolar(
                       0.3, 0.2, 0.92, 120000.0)
                       .construct_ray(geometry.coordinates()));
    rays.push_back(sasktran2::viewinggeometry::GroundViewingSolar(
                       0.3, 0.2, 0.98, 23300.0)
                       .construct_ray(geometry.coordinates()));
    rays.push_back(sasktran2::viewinggeometry::ViewingUpSolar(
                       0.3, 0.2, 0.75, 12300.0)
                       .construct_ray(geometry.coordinates()));
...
TEST_CASE("Integrated 1D OD weights - plane parallel paths",
    ...
    std::vector<sasktran2::viewinggeometry::ViewingRay> rays;
    rays.push_back(sasktran2::viewinggeometry::GroundViewingSolar(
                       0.3, 0.2, 0.45, 120000.0)
                       .construct_ray(geometry.coordinates()));
    rays.push_back(sasktran2::viewinggeometry::GroundViewingSolar(
                       0.3, 0.2, 0.83, 23300.0)
                       .construct_ray(geometry.coordinates()));
    rays.push_back(sasktran2::viewinggeometry::ViewingUpSolar(
                       0.3, 0.2, 0.37, 12300.0)
                       .construct_ray(geometry.coordinates()));
...
TEST_CASE("Integrated 1D OD weights - traced result reuse",
    ...
    auto limb = sasktran2::viewinggeometry::TangentAltitude(3900.0, 0.2,
                                                            120000.0)
                    .construct_ray(geometry.coordinates());
    ...
    auto empty = sasktran2::viewinggeometry::ViewingUpSolar(0.3, 0.2, 0.8,
                                                            120000.0)
                     .construct_ray(geometry.coordinates());
    ...
    auto ground = sasktran2::viewinggeometry::GroundViewingSolar(
                      0.3, 0.2, 0.97, 23300.0)
                      .construct_ray(geometry.coordinates());
...
TEST_CASE("Integrated 1D OD weights - legacy fallback",
    ...
    auto geometry = make_geometry(sasktran2::geometrytype::spherical,
                                  sasktran2::grids::interpolation::linear);
    sasktran2::raytracing::SphericalShellRayTracer tracer(geometry);
    const auto viewing_ray = sasktran2::viewinggeometry::TangentAltitude(
                                 3900.0, 0.2, 120000.0)
                                 .construct_ray(geometry.coordinates());
    ...
    sasktran2::raytracing::construct_od_matrix(legacy, geometry,
                                               fallback_matrix);
    REQUIRE(Eigen::MatrixXd(cached_matrix).isApprox(
        Eigen::MatrixXd(fallback_matrix), 2e-13));
...
TEST_CASE("Integrated 1D OD weights - Rust adapter",
    ...
    const auto ground = sasktran2::viewinggeometry::GroundViewingSolar(
                            0.3, 0.2, 0.97, 23300.0)
                            .construct_ray(geometry.coordinates());
