...
namespace sasktran2::viewinggeometry {
    ...
    FluxObserver FluxObserverSolar::construct_flux_observer(
        const sasktran2::Coordinates& geometry) {
            ...
            flux_observer.observer.position = geometry.solar_coordinate_vector(m_cos_sza, 0.0, m_observer_altitude);
    ...
    std::string FluxObserverSolar::to_string() const {
        return fmt::format(
            "FluxObserverSolar(cos_sza={}, observer_altitude={})",
            m_cos_sza, m_observer_altitude);
    }
}