...
namespace sasktran2::atmosphere {
    ...
    template<int NSTOKES, bool ssonly>
    void PhaseInterpolator<NSTOKES, ssonly>::load_scattering_angle(int num_legendre,
        ...
        if constexpr (NSTOKES == 3) {
            ...
            m_scattering_weights.resize(m_scattering_weights.rows(), 4*num_legendre);
    ...
    void PhaseInterpolator<NSTOKES, ssonly>::scatter(const AtmosphereGridStorageFull<NSTOKES>& phase_storage,
                                                     int wavelidx,
        ...
        Eigen::Map<const Eigen::MatrixXd> phase_matrix(&phase_storage.leg_coeff(0, 0, wavelidx), d[0], d[1] );
        ...
        if constexpr(NSTOKES == 1) {
                ...
                if(source.deriv.size() > 0) {
                    for(const auto& ele : index_weights) {
                        for(int i = 0; i < numscatderiv; ++i) {
                            Eigen::Map<const Eigen::MatrixXd> d_phase_matrix(&phase_storage.d_leg_coeff(0, 0, wavelidx, i), d[0], d[1] );


                            source.deriv(Eigen::all, derivstart + i*d[1] + ele.first) += (ele.second * source.value(0) * (m_scattering_weights * d_phase_matrix(Eigen::all, ele.first))).template cast<double>();
                        }
                    }
                }
            } else {
            ...
            for(const auto& ele : index_weights) {
                phase_result += ele.second * (m_scattering_weights.dot(phase_matrix(Eigen::all, ele.first)));
            }
            ...
            for(const auto& ele : index_weights) {
                phase_result.array() += ele.second * (m_scattering_weights * phase_matrix(Eigen::all, ele.first)).array();
            }
    ...
    template void PhaseInterpolator<1, false>::scatter(const AtmosphereGridStorageFull<1>&, int, const std::vector<std::pair<int, double>>&, sasktran2::Dual<double, sasktran2::dualstorage::dense, 1>&) const;
    template void PhaseInterpolator<3, false>::scatter(const AtmosphereGridStorageFull<3>&, int, const std::vector<std::pair<int, double>>&, sasktran2::Dual<double, sasktran2::dualstorage::dense, 3>&) const;

    template void PhaseInterpolator<1, true>::scatter(const AtmosphereGridStorageFull<1>&, int, const std::vector<std::pair<int, double>>&, sasktran2::Dual<double, sasktran2::dualstorage::dense, 1>&) const;
    template void PhaseInterpolator<3, true>::scatter(const AtmosphereGridStorageFull<3>&, int, const std::vector<std::pair<int, double>>&, sasktran2::Dual<double, sasktran2::dualstorage::dense, 3>&) const;
