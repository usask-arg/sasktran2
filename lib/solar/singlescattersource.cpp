...
#pragma omp parallel for num_threads(m_config->num_threads())
        for (int i = 0; i < (*m_los_rays).size(); ++i) {
            ...
            for (int j = 0; j < ray.layers.size(); ++j) {
                ...
                m_phase_interp[m_phase_index_map[i][j]].load_scattering_angle(
                    m_atmosphere->storage().max_stored_legendre(),
        ...
        for(int losidx = 0; losidx < m_los_rays->size(); ++losidx) {
            for(int layeridx = 0; layeridx <
        (*m_los_rays)[losidx].layers.size(); ++layeridx) {
        ...
        m_phase_interp[m_phase_index_map[losidx][layeridx]];
                ...
                double entrance_factor = exp(-solar_od_entrance) /
        (sasktran2::math::Pi * 4); double exit_factor = exp(-solar_od_exit) /
        (sasktran2::math::Pi * 4);
                ...
                for(auto& ele : layer.entrance.interpolation_weights) {
                    ssa_start += m_atmosphere->storage().ssa(ele.first,
        wavelidx) * ele.second;
                }
        ...
        * ele.second;
                ...
                phase_interp.scatter(m_atmosphere->storage().phase[wavelidx],
        layer.entrance.interpolation_weights, start_phase);
                phase_interp.scatter(m_atmosphere->storage().phase[wavelidx],
        ...
        factors for (Eigen::SparseMatrix<double, Eigen::RowMajor>::InnerIterator
        it(m_geometry_sparse, entrance_index); it; ++it) {
                                start_phase.deriv(Eigen::all, it.index()) -=
        it.value() * start_phase.value;
                            }

                            for (Eigen::SparseMatrix<double,
        Eigen::RowMajor>::InnerIterator it(m_geometry_sparse, exit_index); it;
        ++it) { end_phase.deriv(Eigen::all, it.index()) -= it.value() *
        end_phase.value;
                            }
                        }
                    }
        ...
        m_atmosphere->ssa_deriv_start_index() + ele.first) += ele.second *
        start_phase.value / ssa_start;
                    }
                    for(auto& ele : layer.exit.interpolation_weights) {
                        end_phase.deriv(Eigen::all,
        m_atmosphere->ssa_deriv_start_index() + ele.first) += ele.second *
        end_phase.value / ssa_end;
                    }
                }
                ...
                // The source factor term will only have extinction derivatives,
        the phase term will have
                // local SSA/scattering derivatives and is ~dense in a 1D
        atmosphere

                source.value.array() += source_factor1 *
        start_phase.value.array() * layer.od_quad_start_fraction;
                source.value.array() += source_factor1 * end_phase.value.array()
        * layer.od_quad_end_fraction;

                if(calculate_derivative) {
                    // Now for the derivatives, start with dsource_factor which
        is sparse for(auto it = shell_od.deriv_iter; it; ++it) {
                        source.deriv(Eigen::all, it.index()).array() +=
        it.value() * (1 - source_factor1) * start_phase.value.array() *
        layer.od_quad_start_fraction; source.deriv(Eigen::all,
        it.index()).array() += it.value() * (1 - source_factor1) *
        end_phase.value.array() * layer.od_quad_end_fraction;
                    }
                    // And add on d_phase
                    source.deriv.array() += source_factor1 *
        start_phase.deriv.array() * layer.od_quad_start_fraction +
        source_factor1 * end_phase.deriv.array() * layer.od_quad_end_fraction;
                }
    ...
    void SingleScatterSource<S, NSTOKES>::integrated_source_constant(
        ...
        phase_interp.scatter(m_atmosphere->storage(), wavelidx,
                             layer.entrance.interpolation_weights, start_phase);
        phase_interp.scatter(m_atmosphere->storage(), wavelidx,
                             layer.exit.interpolation_weights, end_phase);
        ...
        if (calculate_derivative) {
            if constexpr (std::is_same_v<S, SolarTransmissionExact>) {
                if (m_config->wf_precision() !=
                    ...
                    for (Eigen::SparseMatrix<double,
                                             Eigen::RowMajor>::InnerIterator
                             it(m_geometry_sparse, entrance_index);
                         it; ++it) {
                        start_phase.deriv(Eigen::all, it.index()) -=
                            it.value() * start_phase.value;
                    }
