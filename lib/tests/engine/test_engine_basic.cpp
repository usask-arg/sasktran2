...
TEST_CASE("Single Scatter, Rayleigh only SASKTRAN-HR Compare", "[sasktran2][engine]") {
...
TEST_CASE("Spherical Correction", "[sasktran2][engine]") {
    ...
    omp_set_num_threads(1);
    // Construct the geometry
    sasktran2::Coordinates coords(0.99, sasktran2::math::PiOver2, 6372000, sasktran2::geometrytype::spherical);
    ...
    sasktran2::atmosphere::Atmosphere<3> atmo(std::move(storage), std::move(surface), true);
    ...
    for(int i = 0; i < nwavel; ++i) {
        ...
        atmo.storage().phase[i].storage()(11, Eigen::all).setConstant(sqrt(6) / 2);
