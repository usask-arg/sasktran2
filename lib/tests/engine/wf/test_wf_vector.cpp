...
TEST_CASE("Verify k WF Vector", "[sasktran2][engine][wf]") {
    ...
    for (int i = 0; i < nlos; ++i) {
        los.emplace_back(
            std::make_unique<sasktran2::viewinggeometry::TangentAltitude>(
                20000.0 + i * (10000.0 / double(nlos)), 0, 200000));
    }
    ...
    for (int i = 0; i < geo.size(); ++i) {
        double dk = atmo.storage().total_extinction(i, 0) * dk_f;
        ...
        // We will have nlos*NSTOKES wf's to check
        for (int j = 0; j < nlos * 3; ++j) {
            ...
            double numeric_wf = (output_perturb_above.radiance().value(j) -
                                ...
                                (2 * dk);
            double analytic_wf = output.radiance().deriv(j, i);
...
TEST_CASE("Verify ssa WF Vector", "[sasktran2][engine][wf]") {
    ...
    config.set_num_do_streams(4);
    config.set_num_stokes(3);
    config.set_stokes_basis(sasktran2::Config::StokesBasis::solar);
...
TEST_CASE("Verify leg_coeff WF Vector", "[sasktran2][engine][wf]") {
    ...
    atmo.storage().d_leg_coeff.chip(lidx_check, 0).setConstant(1);
    ...
    for (int i = 21; i < geo.size(); ++i) {
        ...
        atmo.storage().leg_coeff(lidx_check, i, 0) += dssa;

        engine.calculate_radiance(atmo, output_perturb_above);
        atmo.storage().leg_coeff(lidx_check, i, 0) -= 2 * dssa;
        ...
        for (int j = 0; j < nlos * 3; ++j) {
            ...
            double numeric_wf = (output_perturb_above.radiance().value(j) -
                                 output_perturb_below.radiance().value(j)) /
                                (2 * dssa);
            double analytic_wf = output.radiance().deriv(j, i + 2 * geo.size());
