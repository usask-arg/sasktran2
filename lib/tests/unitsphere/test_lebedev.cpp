...
TEST_CASE("Lebedev Initialization", "[sasktran2][unitsphere]") {
    ...
    /*
        const std::vector<int> invalid_points = {1, 2, 3, 4, 5, 7, 8, 9, 10, 11,
       12, 13, 15, 16, 17, 18, 19, 20}; for (const auto& npoints :
       invalid_points) {
...
int npoints = 38; for (int l = 0; l <= max_order; l++) { for (int m = -l; m <=
l; m++) { sasktran2::math::LebedevSphere sphere(npoints); Eigen::Matrix<double,
1, -1> values; values.resize(1, npoints); for (int i = 0; i < npoints; i++) {
                const auto pos = sphere.get_quad_position(i);
                double theta = atan2(sqrt(pos.x() * pos.x() + pos.y() *
pos.y()), pos.z()); double phi = atan2(pos.y(), pos.x()); values(0, i) =
