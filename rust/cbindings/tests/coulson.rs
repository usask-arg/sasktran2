...
fn test_coulson() -> Result<()> {
    ...
    atmosphere.storage.leg_coeff.slice_mut(s![8, .., ..]).fill(1.0 / 2.0);
    atmosphere.storage.leg_coeff.slice_mut(s![9, .., ..]).fill(3.0);
    atmosphere.storage.leg_coeff.slice_mut(s![11, .., ..]).fill((6.0 as f64).sqrt() * -1.0 / 2.0);
