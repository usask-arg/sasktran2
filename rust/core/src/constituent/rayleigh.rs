...
impl Rayleigh {
    ...
    pub fn with_manual_xs(mut self, xs: Array1<f64>, king: Array1<f64>, wavelengths_nm: Array1<f64>) -> Self {
...
impl Constituent for Rayleigh {
                    ...
                    .for_each(|extinction, ssa, mut legendre, ndens| {
                        ...
                        *ssa += sigma * ndens;
                        ...
                        if num_stokes == 1 {
                            ...
                            legendre[11] +=
                                sigma * ndens * (6.0 as f64).sqrt() * ((1.0 - delta) / (2.0 + delta));
                        } else {
    ...
    fn register_derivatives<'a>(
            ...
            for (deriv_name, deriv_val) in deriv_names.iter().zip(deriv_vals.iter()) {
                ...
                let mut deriv = derivative_generator.get_derivative_mapping(&full_name).with_scatterer();
                {
