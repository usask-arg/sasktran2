...
impl PyXsecAbsorber {
    ...
    pub fn from_files(filepaths: Vec<String>) -> PyResult<Self> {
        ...
        for filepath in filepaths {
            let path = std::path::PathBuf::from(filepath);
            let db = xsec_absorber::read_fwf_xsec(path.clone())
                .ok_or_else(|| PyValueError::new_err(format!("Failed to read file: {}", path.display())))?;
    ...
    pub fn from_folder(folder: &str) -> PyResult<Self> {
        ...
        if db.0.is_empty() {
            return Err(PyValueError::new_err("No cross section data was loaded from folder"));
        }
    ...
    fn optical_derivatives<'py>(
        ...
        let oq = self
            ...
            .map_err(|e| PyValueError::new_err(format!("Failed to get optical derivatives: {e}")))?;
