...
use crate::prelude::*;
use crate::math::wigner::{self, WignerDCalculator};
...
impl MieIntegrator {
    pub fn new(cos_angles: ArrayView1<f64>, num_legendre: usize, num_threads: usize) -> Result<Self> {
        ...
        Zip::from(cos_angles)
            ...
            .for_each(|cos_theta, mut lpoly_00, mut lpoly_22, mut lpoly_2m2, mut lpoly_02| {
    ...
    pub fn integrate(&self,
        ...
        Zip::indexed(xs_total)
            .and(xs_scattering)
            .for_each(|i, mut xs_total, mut xs_scattering| {
                ...
                Zip::indexed(size_param)
                    ...
                    .for_each(|j, &size_param, &size_weight, &pdf| {
                        *xs_total += size_weight * pdf * std::f64::consts::PI * output.Qext[j] * (
                            size_param * wavelength / (2.0 * std::f64::consts::PI)
                        ).powf(2.0);

                        *xs_scattering += size_weight * pdf * std::f64::consts::PI * output.Qsca[j] * (
                            size_param * wavelength / (2.0 * std::f64::consts::PI)
                        ).powf(2.0);
                    });
                ...
                .for_each(|j, &size_param, &size_weight, &pdf| {
                    ...
                    Zip::from(S1)
                        ...
                        .for_each(|s1, s2, p11, p12, p33, p34| {
                            *p11 += phase_norm * size_weight * pdf * (s1.norm_sqr() + s2.norm_sqr());
                            *p12 += phase_norm * size_weight * pdf * (s1.norm_sqr() - s2.norm_sqr());
                            *p33 += phase_norm * size_weight * pdf * (s1 * s2.conj() + s2 * s1.conj()).re;
                            *p34 += phase_norm * size_weight * pdf * (s1 * s2.conj() - s2 * s1.conj()).im;
                        });
                    });
