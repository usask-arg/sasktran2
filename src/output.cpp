...
void declareOutputIdeal(py::module_ & m, const std::string & suffix) {
    ...
    py::class_<Output , sasktran2::Output<NSTOKES>>(m, ("OutputIdeal" + suffix).c_str())
            .def(py::init<>())
            .def_property("radiance",
                          [](Output& output) -> Eigen::VectorXd& { return output.radiance().value; },
                          nullptr
            )
            .def_property("d_radiance",
                          [](Output& output) -> Eigen::MatrixXd& { return output.radiance().deriv; },
                          nullptr
            )
            ;
}
