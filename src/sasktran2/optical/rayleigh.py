...
def _o2_refrac_bates(wavelengths_um: np.array) -> np.array:
    ...
    """
    coeff_ranges = [{'coeff': [23796.7, 168988.4], 'range': [0, 0.221]},
    ...
    for ele in coeff_ranges:
        ...
        result[good] = ele['coeff'][0] + ele['coeff'][1] / (40.9 - wavelengths_um[good]**(-2))
...
def _n2_refrac_bates(wavelengths_um: np.array) -> np.array:
    ...
    """
    coeff_ranges = [{'coeff': [6998.749, 3233582.0], 'range': [0, 0.254]},
    ...
    for ele in coeff_ranges:
        ...
        result[good] = ele['coeff'][0] + ele['coeff'][1] / (144 - wavelengths_um[good]**(-2)) + 2.27684009 * np.sign(delta_lambda) * np.exp(-np.abs(delta_lambda) / 0.003)
...
def _co2_refrac_bates(wavelengths_um: np.array) -> np.array:
    ...
    """
    return 22822.1 + 117.8 * wavelengths_um**(-2) + 2406030 / (130 - wavelengths_um**(-2)) + 15997 / (38.9 - wavelengths_um**(-2))
...
def rayleigh_cross_section_bates(wavelengths_um: np.array,
    ...
    """
    lorenz_factors =\
          o2_percentage/100 * _o2_refrac_bates(wavelengths_um)**2 * _o2_king_bates(wavelengths_um) +\
          n2_percentage/100 * _n2_refrac_bates(wavelengths_um)**2 * _n2_king_bates(wavelengths_um) +\
          ar_percentage/100 * _ar_refrac_bates(wavelengths_um)**2 * _ar_king_bates(wavelengths_um) +\
          co2_percentage/100 * _co2_refrac_bates(wavelengths_um)**2 * _co2_king_bates(wavelengths_um)
    ...
    num_dens = pressure_temperature_to_numberdensity(1013.25 * 100, celsius_to_kelvin(0))

    return 32 * np.pi**3 / (3*num_dens**2*wavelengths_um**4) * (lorenz_factors) * 1e8, eff_king
