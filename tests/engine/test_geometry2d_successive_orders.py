...
def test_explicit_midpoint_grid_preserves_default_solution():
    ...
    default_result = sk.Engine(
        default_config, geometry, viewing
    ).calculate_radiance(homogeneous_atmosphere(geometry, default_config))
    explicit_result = sk.Engine(
        explicit_config, geometry, viewing
    ).calculate_radiance(homogeneous_atmosphere(geometry, explicit_config))
...
def test_native_products_resolve_horizontal_atmosphere_nodes():
    ...
    atmosphere = homogeneous_atmosphere(
        geometry, config, calculate_derivatives=True
    )
    horizontal_factor = np.linspace(0.9, 1.1, geometry.shape[0])[:, np.newaxis]
    location_factor = np.repeat(
        horizontal_factor, geometry.shape[1], axis=1
    ).reshape(-1, 1)
    ...
    tangent["ssa"].data[:] = np.linspace(
        -0.02, 0.03, tangent["ssa"].size
    ).reshape(tangent["ssa"].shape)
    jvp = linearization.jvp(tangent)

    cotangent = xr.ones_like(linearization.value)
    cotangent.data[:] = np.linspace(0.4, 1.0, cotangent.size).reshape(
        cotangent.shape
    )
    ...
    for sign in (-1.0, 1.0):
        ...
        perturbed.storage.total_extinction[:] += (
            sign
            * epsilon
            * tangent["extinction"].values.reshape(-1, 1)
        )
        perturbed.storage.ssa[:] += (
            sign * epsilon * tangent["ssa"].values.reshape(-1, 1)
        )
        perturbed_radiances.append(
            engine.calculate_radiance(perturbed).radiance.values
        )
    finite_difference = (
        perturbed_radiances[1] - perturbed_radiances[0]
    ) / (2.0 * epsilon)
    np.testing.assert_allclose(
        jvp.values, finite_difference, rtol=5.0e-5, atol=2.0e-9
    )
