...
# header line numbers
groups = [dict(header_line_number=ln) for ln in [1, 4004, 8007, 12010, 16013, 20016, 24019, 28022, 32025, 36028, 40031, 44034, 48037, 52040, 56043, 60046, 64301, 64808, 65526, 68099, 68793, 70169, 71545, 72919, 74295, 87387, 100366, 113414, 126550]]
...
for g in groups:
    data = np.loadtxt(fname=fname_in, skiprows=g['header_line_number'], max_rows=g['size'])
...
regions = [list(range(15)), [15], [16], [17], [18], [19], list(range(20, 24)), list(range(24, 29))]
for region in regions:
    t0 = np.array([groups[k]['temperature'] for k in region])  # temperatures of given data
    for i in range(len(all_t)):
        ...
        if t in t0:  # insert data where all_t matches temperatures of given data 
            ...
            all_xs[i, j] = xs
        elif t > np.min(t0) and t < np.max(t0):  # interpolate where all_t does not match but is bounded by the temperatures of the given data
            # bound indices
            ...
            k = region[0] + (len(t0) - 1 if t > np.max(t0) else 0)  # copy min temp data if t < min temp, max temp data if t > max temp
            wn = groups[k]['wavenumber']
            xs = groups[k]['cross_section']
            j = np.searchsorted(all_wn, wn, side='left')
            all_xs[i, j] = xs
